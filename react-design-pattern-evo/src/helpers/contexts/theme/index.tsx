...
export const ThemeContext = createContext<AppThemeContext | undefined>(undefined);
...
export default function AppThemeProvider({ children }: React.PropsWithChildren) {
  ...
  const setAppTheme = (): void => {
    setTheme(() => {
      if (!isLightTheme()) document.documentElement.classList.replace('dark', 'light');
  ...
  useEffect(() => {
    if (!isLightTheme()) document.documentElement.classList.add('dark');
